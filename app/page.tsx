...
export default function Home() {
  ...
  function saveCurrentGroup() {
    if (!userId) { setError("Sign in to save a group."); setAccountOpen(true); return; }
  ...
  async function createShareLink() {
    ...
    try {
      if (!supabase || !userId) throw new Error("Sign in to create a private sharing link.");
  ...
  async function paypalHeaders() {
    ...
    if (!token) throw new Error("Sign in before connecting PayPal.");
  ...
  return <main className={`${draft.theme} ${guestParticipantId ? "guest-mode" : ""}`}><div className="app-shell">
    ...
    {accountOpen && <div className="account-backdrop" role="presentation" onMouseDown={() => setAccountOpen(false)}><section className="account-panel panel" role="dialog" aria-modal="true" aria-label="Account" onMouseDown={(event) => event.stopPropagation()}><button className="account-close" onClick={() => setAccountOpen(false)}>×</button><img src="/bill-splitter-icon.png" alt="" /><h2>{userEmail ? "Your account" : "Save your bills everywhere"}</h2>{userEmail ? <><p>Signed in as <strong>{userEmail}</strong></p><button className="google-button" onClick={loadHistory}>My Bills</button><button className="restaurant-settings-button" onClick={()=>{setAccountOpen(false);setRestaurantOpen(true);}}>{ownRestaurant?"Restaurant & payment settings":"Register your restaurant"}</button><button className="auth-secondary" onClick={signOut}>Sign out</button></> : supabaseConfigured ? <><button className="google-button" onClick={signInWithGoogle}>Continue with Google</button><div className="auth-divider"><span>or</span></div><label><span>Email address</span><input type="email" value={authEmail} onChange={(event) => setAuthEmail(event.target.value)} placeholder="you@example.com" /></label><button className="email-button" onClick={sendMagicLink}>Email me a sign-in link</button>{authMessage && <p className="auth-message">{authMessage}</p>}<button className="auth-secondary" onClick={() => setAccountOpen(false)}>Continue without account</button></> : <><p>Cloud accounts are ready to connect. Add your Supabase Project URL and public anonymous key to enable Google and email sign-in.</p><div className="local-mode">Local mode active</div><button className="auth-secondary" onClick={() => setAccountOpen(false)}>Continue without account</button></>}</section></div>}
    ...
    {groupsOpen && <div className="account-backdrop" role="presentation" onMouseDown={() => { setGroupsOpen(false); setEditingGroup(null); }}><section className="groups-panel panel" role="dialog" aria-modal="true" aria-label="Saved groups" onMouseDown={(event) => event.stopPropagation()}><button className="account-close" onClick={() => { setGroupsOpen(false); setEditingGroup(null); }}>×</button><h2>{editingGroup ? (editingGroup.id ? "Edit saved group" : "Save new group") : "Saved groups"}</h2>
      {!userId ? <div className="group-signin"><p>Sign in to securely save groups with phone numbers and Venmo usernames.</p><button className="google-button" onClick={() => { setGroupsOpen(false); setAccountOpen(true); }}>Sign in</button></div> : editingGroup ? <div className="group-editor">
    ...
    {draft.step === 1 && <section className="start-page">
      ...
      <div className="start-options">
        ...
        <article className={`panel start-option start-account ${userId?"account-on":"account-off"}`}><div className="start-account-top"><span className="start-option-icon" aria-hidden="true">✓</span><span className="account-state"><i></i>{userId?"ON · Signed in":"OFF · Sign in required"}</span></div><div><small>OPTIONAL ACCOUNT</small><h2>{userId?"Your saved bills":"Saved bills are locked"}</h2><p>{userId?<>Signed in as <strong>{userEmail}</strong>. Open saved groups and previous bills, or begin a new restaurant bill.</>:"Sign in to turn on saved groups and keep your bill history available on your devices."}</p></div>{userId?<div className="start-account-actions"><button className="auth-secondary" onClick={()=>{setEditingGroup(null);setGroupsOpen(true);}}>Saved groups</button><button className="auth-secondary" onClick={()=>void loadHistory()}>Saved bills &amp; history</button><button className="google-button" onClick={()=>goTo(2)}>Start a new bill</button><button className="auth-secondary start-signout" onClick={()=>void signOut()}>Log out</button></div>:<button className="google-button" onClick={()=>setAccountOpen(true)}>Sign in to turn this on</button>}</article>
      </div>
    </section>}
