...
export default function Home() {
  ...
  return <main className={`${draft.theme} ${guestParticipantId ? "guest-mode" : ""}`}><div className="app-shell">
    ...
    {draft.step === 1 && <section className="start-page">
      <section className="page-title start-title"><span className="eyebrow">RESTAURANT BILL SPLITTER</span><h1>How would you like to begin?</h1><p>You can continue as a guest without creating an account. Log in only when you want saved groups and bill history.</p></section>
      <div className="start-options">
        <article className="panel start-option start-guest"><div className="start-option-heading"><span className="start-option-icon" aria-hidden="true">$</span><small>QUICK START</small></div><div><h2>Split &amp; Pay as a guest</h2><p>Add people, scan or enter the restaurant bill, assign items, and share the final results. No account required.</p></div><button className="calculate" onClick={()=>goTo(2)}>Continue as guest <span className="nav-arrow">›</span></button></article>
        <article className={`panel start-option start-account ${userId?"account-on":"account-off"}`}><div className="start-option-heading"><span className="start-option-icon" aria-hidden="true">✓</span><small>OPTIONAL ACCOUNT</small></div><div><h2>Saved groups &amp; bill history</h2><p>{userId?<>Logged in as <strong>{userEmail}</strong>. Your saved account features are available.</>:"Log in to reuse groups and keep your bill history available on your devices."}</p></div><div className="start-account-actions"><button className="saved-feature-button" disabled={!userId} onClick={()=>{setEditingGroup(null);setGroupsOpen(true);}}>Saved groups</button><button className="saved-feature-button" disabled={!userId} onClick={()=>void loadHistory()}>Saved bills &amp; history</button>{userId?<><button className="google-button" onClick={()=>goTo(2)}>Start a new bill</button><button className="auth-secondary start-signout" onClick={()=>void signOut()}>Log out</button></>:<button className="google-button" onClick={()=>setAccountOpen(true)}>Log in</button>}</div></article>
      </div>
    </section>}
