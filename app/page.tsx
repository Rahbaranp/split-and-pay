...
export default function Home() {
  ...
  async function shareResults() {
    ...
    try {
      ...
      // The organizer uses the same private results flow as every other person.
      // After sharing, ask them to select their own name before revealing a personal result.
      setCloudShareToken(token);
      setShareLink(resultsLink);
      window.history.replaceState({}, "", resultsLink);
    } catch (cause) {
      if (cause instanceof DOMException && cause.name === "AbortError") return;
  ...
  return <main className={`${draft.theme} ${guestParticipantId ? "guest-mode" : ""} ${draft.step===3?"expenses-step":""}`}><div className="app-shell">
    ...
    {groupsOpen && <div className="account-backdrop" role="presentation" onMouseDown={() => { setGroupsOpen(false); setEditingGroup(null); }}><section className="groups-panel panel" role="dialog" aria-modal="true" aria-label="Saved groups" onMouseDown={(event) => event.stopPropagation()}>{!editingGroup&&<><button className="account-close" onClick={() => { setGroupsOpen(false); setEditingGroup(null); }}>×</button><h2>Saved groups</h2></>}
      {!userId ? <div className="group-signin"><p>Log in to securely save groups with phone numbers and Venmo usernames.</p><button className="google-button" onClick={() => { setGroupsOpen(false); setAccountOpen(true); }}>Log in</button></div> : editingGroup ? <div className="group-editor"><div className="group-editor-scroll">
        ...
        <div className="group-member-list">{editingGroup.people.map((person,index)=><details className="group-member" key={person.id}><summary><input className="group-check" type="checkbox" checked={person.selected!==false} onClick={(event)=>event.stopPropagation()} onChange={(event)=>setEditingGroup({...editingGroup,people:editingGroup.people.map((member,i)=>i===index?{...member,selected:event.target.checked}:member)})}/><strong>{person.name||"New person"}</strong><span className="group-member-chevron">⌄</span></summary><div className="group-member-fields"><input value={person.name} onChange={(event)=>setEditingGroup({...editingGroup,people:editingGroup.people.map((member,i)=>i===index?{...member,name:event.target.value}:member)})} placeholder="Name"/><input type="tel" value={person.phone} onChange={(event)=>setEditingGroup({...editingGroup,people:editingGroup.people.map((member,i)=>i===index?{...member,phone:event.target.value}:member)})} placeholder="Phone number"/><div className="group-venmo"><span>@</span><input value={person.venmoUsername} onChange={(event)=>setEditingGroup({...editingGroup,people:editingGroup.people.map((member,i)=>i===index?{...member,venmoUsername:event.target.value.replace(/^@/,"")}:member)})} placeholder="Venmo username"/></div></div></details>)}</div></div>
        <div className={`group-editor-actions ${editingGroup.id?"has-delete":""}`}>{editingGroup.id&&<button className="danger-button group-delete" onClick={()=>void deleteSavedGroup(editingGroup.id)}>Delete</button>}<button className="group-add-person" onClick={()=>setEditingGroup({...editingGroup,people:[...editingGroup.people,{id:crypto.randomUUID(),name:"",phone:"",venmoUsername:"",color:COLORS[editingGroup.people.length%COLORS.length],selected:true}]})}>＋ Add person</button><button className="auth-secondary group-back" onClick={()=>setEditingGroup(null)}>Back</button><button className="auth-secondary group-save-changes" disabled={groupBusy} onClick={()=>void saveEditingGroup()}>{groupBusy?"Saving…":"Save changes"}</button><button className="google-button group-use-selected" disabled={editingGroup.people.filter((person)=>person.selected!==false).length<2} onClick={()=>chooseGroup(editingGroup)}>Use selected</button></div>
    ...
    {draft.step === 1 && <section className="start-page">
      {/* Keep both entry choices compact and immediately visible on the start screen. */}
      <div className="start-options">
        <article className={`panel start-option start-account ${userId?"account-on":"account-off"}`}><div><h2>Saved groups &amp; bill history</h2><p>{userId?<>Logged in as <strong>{userEmail}</strong>. Your saved account features are available.</>:"Log in to reuse saved groups and your bill history on any device."}</p></div><div className="start-account-actions">{userId?<><button className="saved-feature-button" onClick={()=>{setEditingGroup(null);setGroupsOpen(true);}}>Saved groups</button><button className="saved-feature-button" onClick={()=>void loadHistory()}>Saved bills &amp; history</button><button className="google-button" onClick={()=>goTo(2)}>Start a new bill</button><button className="auth-secondary start-signout" onClick={()=>void signOut()}>Log out</button></>:<button className="google-button" onClick={()=>setAccountOpen(true)}>Log in</button>}</div></article>
        <article className="panel start-option start-guest"><div><h2>Split &amp; Pay as a guest</h2><p>Split a restaurant bill with friends. No account required.</p></div><button className="calculate" onClick={()=>goTo(2)}>Continue as guest</button></article>
      </div>
    </section>}
